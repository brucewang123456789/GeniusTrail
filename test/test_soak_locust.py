...
try:
    ...
    locust_events = types.SimpleNamespace(
        test_start=types.SimpleNamespace(add_listener=lambda f: None)
    )
...
def test_placeholder_skip():
    ...
    pytest.skip(
        "Skipping soak load test in pytest environment", allow_module_level=False
    )
