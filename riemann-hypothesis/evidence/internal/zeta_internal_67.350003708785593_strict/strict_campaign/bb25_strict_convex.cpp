...
static bool prove(Box B){
  ...
  // High-cost but fully rigorous gate only for narrow boxes. This uses direct
  // interval evaluation of Wall() and cannot weaken soundness: a failure merely
  // falls through to the existing bounds.
  if(prove_strong_convex_direct(B)) return true;
