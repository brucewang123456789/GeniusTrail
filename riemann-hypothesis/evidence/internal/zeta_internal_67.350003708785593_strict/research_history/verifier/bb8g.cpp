...
static std::vector<float> Wmn,W1mx,W2mn,Wmid,W1mid,W2ax;           // per cell
...
static void build(){
  ...
  Wmid.resize(NCELL);W1mid.resize(NCELL);W2ax.resize(NCELL);
  for(int i=0;i<NCELL;i++){
    ...
    W2ax[i]=(float)up(std::max(std::fabs(W2.lo),std::fabs(W2.hi)));
    double md=(i+0.5)*HC; I Wm,W1m,W2m; Wall(mk(md,md),Wm,W1m,W2m);
    Wmid[i]=(float)dn(Wm.lo); W1mid[i]=(float)((W1m.lo+W1m.hi)*0.5);}
...
static inline double Wpt_lo(double d){          // rigorous 2nd-order table lower bound at a point
  int i=(int)(d/HC); if(i<0)i=0; if(i>=NCELL)i=NCELL-1;
  double md=(i+0.5)*HC, de=d-md;
  double lin=(double)Wmid[i]+(double)W1mid[i]*de;
  double err=up(std::fabs((double)W1mx[i]-std::fabs((double)W1mid[i]))*std::fabs(de))
           + up(0.5*(double)W2ax[i]*de*de);
  return dn(lin-err);
}
static inline double W1pt_abs(double d){        // rigorous |W'(d)| upper bound at a point
  int i=(int)(d/HC); if(i<0)i=0; if(i>=NCELL)i=NCELL-1;
  double md=(i+0.5)*HC, de=d-md;
  return up(std::fabs((double)W1mid[i])+std::fabs((double)W2ax[i]*de));
}
static inline double W1pt(double d,double&err){
  int i=(int)(d/HC); if(i<0)i=0; if(i>=NCELL)i=NCELL-1;
  double md=(i+0.5)*HC, de=d-md;
  err=up(std::fabs((double)W2ax[i]*de)); return (double)W1mid[i];
}
static double SLOPE,EPS;
...
static double WMIN=1e-3; static std::vector<Box> HARD; static double worstlb=1e30;
static bool contract(Box&B){
  for(int it=0;it<3;it++){
    ...
    for(int k=0;k<NG;k++){
      ...
      B.lo[k]=(float)nl; B.hi[k]=(float)nh;
    }
    if(!ch) break;
  }
  return true;
