...
ctx.prec=300
cfg=json.load(open('cfgB_rat.json')); mins={F(k):v for k,v in json.load(open('minsB.json')).items()}
...
q=12; d=24; T=F(13,12); B=F(93,23000); G=10**7
def sq(x,K=10**30):
...
json.dump({"q":12,"m":m,"eta":[eta.numerator,eta.denominator],"H":[Hf.numerator,Hf.denominator],
 ...
 "percent":"%.14f"%(100*float(Cc)),"status":"RESEARCH_DRAFT_PENDING_LOCAL_CERTIFICATES"},open('exactB.json','w'))
