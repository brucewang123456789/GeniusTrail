...
The present release improves the external finite certificate, not the challenge contract. A leaderboard submission still needs a complete Lean proof of the fixed theorem and the exact analytic-interface correspondence. The strict C++ certificate can be reviewed and replayed independently, but it is not automatically a Lean proof.
