...
async def chat(req: ChatRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="LLM backend failure")
...
async def chat_stream(req: ChatRequest):
    ...
    async def gen() -> AsyncIterator[str]:
        ...
        try:
            ...
            raise HTTPException(status_code=status.HTTP_502_BAD_GATEWAY, detail=f"Upstream {e.response.status_code}")
        except Exception:
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Streaming failure")
